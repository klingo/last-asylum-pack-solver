 ...
 * exclusive/event/unlimited) limit actually allows, given the "ignore" checkboxes and the
 * "days" planning horizon:
 * - "exclusive" and "event" limits are NEVER scaled or ignored.
 * - A checked ignore checkbox for a limit type makes that limit unlimited outright.
 ...
 * lifted via its checkbox (not merely scaled by the day count).
 */
...
function collectPackageSources(targetId, packages, items, limitOptions = {}, locale = 'en', activeEventIds = null) {
    const normalizedLimitOptions = normalizeLimitOptions(limitOptions);
    const sources = [];
    for (const [pkgId, pkg] of Object.entries(packages)) {
        if (pkg.event_id && activeEventIds && !activeEventIds.has(pkg.event_id)) {
